  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    if (is_non_idle_command(socket_commands)) {
        ...
        if (socket_commands.forward == 1) {
            printf("GO FORWARD - Setting forward_desired = +0.2\n");
            forward_desired = +0.2;
        }
        if (socket_commands.backward == 1) {
            printf("GO BACKWARD - Setting forward_desired = -0.2\n");
            forward_desired = -0.2;
        }
        if (socket_commands.right == 1) {
            printf("GO RIGHT - Setting sideways_desired = -0.2\n");
            sideways_desired = -0.2;
        }
        if (socket_commands.left == 1) {
            printf("GO LEFT - Setting sideways_desired = +0.2\n");
            sideways_desired = +0.2;
        }
        if (socket_commands.yaw_increase == 1) {
            printf("YAW INCREASE - Setting yaw_desired = 0.2\n");
            yaw_desired = 0.2;
        }
        if (socket_commands.yaw_decrease == 1) {
            printf("YAW DECREASE - Setting yaw_desired = -0.2\n");
            yaw_desired = -0.2;
        }
